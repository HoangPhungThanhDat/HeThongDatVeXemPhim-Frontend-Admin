...
.sf-pill.pill-banned{background:#FCEBEB;border-color:#A32D2D;color:#791F1F;font-weight:500;}
...
@media (prefers-color-scheme:dark){
  ...
  .sf-pill.pill-banned   {background:#2e0f0f;border-color:#A32D2D;color:#F09595;}
}
`;

function injectSearchBarCSS() {
  if (document.getElementById("__sf-user-css")) return;
...
const PILL_CONFIG = [
  ...
  { key: "Banned",   label: "Cấm",        cls: "pill-banned",   dot: "#A32D2D"   },
];
...
// ─── Row ──────────────────────────────────────────────────────────────────────
const UserRow = React.memo(({ user, index, onToggle, onNavigate, onDelete }) => (
  <tr className="table-row-hover">
    ...
    <td>
      <span className={`badge role-badge ${user.role?.RoleName === "Admin" ? "bg-gradient-primary" : "bg-gradient-secondary"}`}>
        {user.role?.RoleName || "User"}
      </span>
    </td>
    <td>
      <label className="switch">
        <input type="checkbox" checked={user.Status === "Active"} onChange={() => onToggle(user.UserId)} />
    ...
    <td className="text-center">
      <button className="action-btn text-info" title="Chi tiết" onClick={() => onNavigate(`/User/${user.UserId}`)}><i className="fas fa-eye" /></button>
      <button className="action-btn text-primary" title="Sửa" onClick={() => onNavigate(`/User/edit/${user.UserId}`)}><i className="fas fa-edit" /></button>
      <button className="action-btn text-danger" title="Xóa" onClick={() => onDelete(user.UserId)}><i className="fas fa-trash" /></button>
    </td>
  </tr>
));
...
export default function User() {
  ...
  const [newUser, setNewUser] = useState({
    ...
    DateOfBirth: "", PasswordHash: "", RoleId: 2, Status: "Active",
  });
  ...
  useEffect(() => {
    RoleApi.getAll().then((r) => setRoles(r.data.data)).catch(console.error);
  }, []);
  ...
  const toggleStatus = useCallback((userId) => {
    const user = users.find((u) => u.UserId === userId);
    if (!user) return;
    UserApi.update(userId, { ...user, Status: user.Status === "Active" ? "Inactive" : "Active" })
      .then(() => reload())
      .catch(() => showToast("error", "❌ Không thể cập nhật trạng thái!"));
  }, [users, reload, showToast]);

  const handleNavigate  = useCallback((path) => navigate(path), [navigate]);
  const handleDelete    = useCallback((id) => deleteUser(id, () => reload()), [reload]);
  const handleClear     = useCallback(() => { setSearchInput(""); setSearch(""); }, []);
  const field           = useCallback((k, v) => setNewUser((p) => ({ ...p, [k]: v })), []);

  const handleAddUser = useCallback(async (e) => {
    e.preventDefault();
    try {
      await UserApi.create(newUser);
      setNewUser({ FullName: "", Email: "", PhoneNumber: "", Gender: "", DateOfBirth: "", PasswordHash: "", RoleId: 2, Status: "Active" });
      setShowForm(false);
      reload();
    ...
    <div><MainLayout><main><div className="main-container"><div className="pd-ltr-20">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4 p-3 shadow-sm bg-gradient rounded-4 header-box">
        <h3 className="m-0 text-white fw-bold d-flex align-items-center">
          <i className="fas fa-users me-2" />Quản lý người dùng
        </h3>
        <div>
      ...
      <div className="mb-2" style={{ fontSize: 13, color: "var(--bs-secondary-color, #6c757d)", minHeight: 20 }}>
        {loading ? (
          ...
          <span>
            {search || statusFilter ? `Tìm thấy ${total} người dùng` : `Tổng cộng ${total} người dùng`}
          </span>
        )}
      </div>
              ...
              <form onSubmit={handleAddUser}>
                <div className="row g-4">
                  <div className="col-md-6">
                    <label className="form-label fw-bold"><i className="fas fa-user me-2 text-primary" />Họ tên</label>
                    <input type="text" className="form-control custom-input" placeholder="Nhập họ và tên" value={newUser.FullName} onChange={(e) => field("FullName", e.target.value)} required />
                  </div>
                  <div className="col-md-6">
                    <label className="form-label fw-bold"><i className="fas fa-envelope me-2 text-danger" />Email</label>
                    <input type="email" className="form-control custom-input" placeholder="Nhập email" value={newUser.Email} onChange={(e) => field("Email", e.target.value)} required />
                  </div>
                  <div className="col-md-6">
                    <label className="form-label fw-bold"><i className="fas fa-phone me-2 text-success" />Số điện thoại</label>
                    <input type="text" className="form-control custom-input" placeholder="Nhập số điện thoại" value={newUser.PhoneNumber} onChange={(e) => field("PhoneNumber", e.target.value)} required />
                  </div>
                  <div className="col-md-6">
                    <label className="form-label fw-bold"><i className="fas fa-venus-mars me-2 text-info" />Giới tính</label>
                    <select className="form-select custom-input" value={newUser.Gender} onChange={(e) => field("Gender", e.target.value)} required>
                  ...
                  <div className="col-md-6">
                    <label className="form-label fw-bold"><i className="fas fa-calendar me-2 text-warning" />Ngày sinh</label>
                    <input type="date" className="form-control custom-input" value={newUser.DateOfBirth} onChange={(e) => field("DateOfBirth", e.target.value)} required />
                  </div>
                  <div className="col-md-6">
                    <label className="form-label fw-bold"><i className="fas fa-lock me-2 text-dark" />Mật khẩu</label>
                    <input type="password" className="form-control custom-input" placeholder="Tối thiểu 8 ký tự" value={newUser.PasswordHash} onChange={(e) => field("PasswordHash", e.target.value)} required minLength={8} />
                  </div>
                  <div className="col-md-6">
                    <label className="form-label fw-bold"><i className="fas fa-user-shield me-2 text-secondary" />Vai trò</label>
                    <select className="form-select custom-input" value={newUser.RoleId} onChange={(e) => field("RoleId", Number(e.target.value))} required>
                      <option value="">-- Chọn vai trò --</option>
                      {roles.map((r) => <option key={r.RoleId} value={r.RoleId}>{r.RoleName}</option>)}
                    </select>
                  </div>
                  <div className="col-md-6">
                    <label className="form-label fw-bold"><i className="fas fa-toggle-on me-2 text-success" />Trạng thái</label>
                    <select className="form-select custom-input" value={newUser.Status} onChange={(e) => field("Status", e.target.value)} required>
                ...
                <div className="col-12 text-end mt-3">
                  <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }} type="submit" className="btn btn-gradient-success me-2 rounded-pill px-4">
                    <i className="fas fa-save me-1" />Lưu
                  </motion.button>
                  <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }} type="button" className="btn btn-gradient-secondary rounded-pill px-4" onClick={() => setShowForm(false)}>
      ...
      <div className="card border-0 shadow-sm rounded-4 animate__animated animate__fadeIn">
        <div className="card-body p-4">
                  ...
                  <tr>
                    <td colSpan="6" className="text-center py-5 text-muted">
                      ...
                      {search
                        ...
                        : statusFilter
                          ? "Không có người dùng nào ở trạng thái này."
          ...
          <CinemaPagination
            ...
            onPageChange={goToPage} label="Người dùng"
          />
        </div>
      </div>
