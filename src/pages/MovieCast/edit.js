...
export default function MovieCastEdit() {
  ...
  useEffect(() => {
    if (CastId) {
      MovieCastApi.getById(CastId)
        ...
        .catch((err) => {
          console.error("❌ Lỗi khi lấy dữ liệu cast:", err);
  ...
  const handleChange = (e) => {
    ...
    setCast((prev) => ({ ...prev, [name]: value }));
  };

  // 🔹 Cập nhật Cast
  const handleSubmit = (e) => {
    e.preventDefault();
    MovieCastApi.update(CastId, cast)
      .then(() => {
        Swal.fire({
          toast: true,
          position: "top-end",
          icon: "success",
          title: "🎉 Cập nhật diễn viên thành công!",
        ...
        }).then(() => navigate("/moviecast"));
      })
      .catch((err) => {
        console.error("❌ Lỗi khi cập nhật cast:", err);
        Swal.fire({
          toast: true,
          position: "top-end",
          icon: "error",
          title: "❌ Cập nhật diễn viên thất bại!",
          showConfirmButton: false,
          timer: 3000,
          timerProgressBar: true,
          customClass: {
        ...
        <div className="main-container">
          <div className="pd-ltr-20">
            <div className="d-flex flex-column align-items-center justify-content-center p-5">
              ...
              ></div>
              <h5 className="text-primary">Đang tải dữ liệu diễn viên...</h5>
      ...
      <div className="modern-cinema-page">
        <div className="cinema-container">
          ...
          <div className="breadcrumb-nav">
            ...
            <span className="breadcrumb-item" onClick={() => navigate("/moviecast")}>
              Diễn viên
            </span>
            <span className="breadcrumb-separator">/</span>
            ...
            <div className="form-section">
              <div className="section-header">
                ...
                <div className="header-text">
                  <h2 className="section-title">Chỉnh Sửa Diễn Viên</h2>
                  <p className="section-subtitle">Cập nhật thông tin diễn viên và vai trò</p>
                </div>
              </div>
              ...
              <div className="form-card">
                ...
                <div className="form-group">
                  ...
                  >
                    ...
                    <option value="Writer">Biên kịch</option>
                  </select>
                </div>
                ...
                <div className="form-group">
                  ...
                  <div className="status-selector">
                    <div
                      className={`status-option ${cast.Status === 'Active' ? 'active' : ''}`}
                      onClick={() => setCast({...cast, Status: 'Active'})}
                    >
                      <div className="status-radio">
                        {cast.Status === 'Active' && <div className="status-dot"></div>}
                      </div>
                    ...
                    <div
                      className={`status-option ${cast.Status === 'Inactive' ? 'active' : ''}`}
                      onClick={() => setCast({...cast, Status: 'Inactive'})}
                    >
                      <div className="status-radio">
                        {cast.Status === 'Inactive' && <div className="status-dot"></div>}
                      </div>
                      <div className="status-content">
            ...
            <div className="info-section">
              <div className="info-card highlight-card">
                ...
                <p className="info-text">
                  Thông tin diễn viên sẽ được hiển thị trong chi tiết phim. 
                  Vui lòng kiểm tra kỹ tên và vai trò trước khi lưu.
                </p>
              </div>
              ...
              <div className="info-card">
                ...
                <div className="info-list">
                  <div className="info-item">
                    <span className="info-key">Tên diễn viên:</span>
                    <span className="info-value">{cast.Name || "Chưa có"}</span>
                  </div>
                  <div className="info-item">
                    <span className="info-key">Vai trò:</span>
                    <span className="info-value">
                      {cast.Role === "Actor" ? "Diễn viên" : 
                       cast.Role === "Director" ? "Đạo diễn" :
                       cast.Role === "Producer" ? "Nhà sản xuất" :
                       cast.Role === "Writer" ? "Biên kịch" : "Chưa có"}
                    </span>
                  </div>
                  <div className="info-item">
                    <span className="info-key">Trạng thái:</span>
                    <span className={`status-pill ${cast.Status === 'Active' ? 'pill-active' : 'pill-inactive'}`}>
                      {cast.Status === 'Active' ? 'Hoạt động' : 'Không hoạt động'}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="info-card tips-card">
                <div className="tip-item">
                  ...
                  <p>Chọn phim trước khi nhập thông tin</p>
                </div>
                <div className="tip-item">
                  <i className="fas fa-check-circle tip-icon"></i>
                  <p>Tên diễn viên nên chính xác và đầy đủ</p>
                </div>
                <div className="tip-item">
                  <i className="fas fa-check-circle tip-icon"></i>
                  <p>Chọn đúng vai trò phù hợp với công việc</p>
                </div>
              </div>
            </div>
          </div>
