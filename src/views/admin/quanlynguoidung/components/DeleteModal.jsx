// src/views/admin/quanlynguoidung/components/DeleteModal.jsx
...
const DeleteModal = ({ isOpen, onClose, user, onConfirm, isDark }) => {
  ...
  const handle = () => {
    if (inputVal !== user.UserId?.toString()) { 
      setShaking(true); 
      ...
      <ModalContent borderRadius="18px" border={isDark ? `1px solid ${DARK.ink5}` : "1px solid #fca5a5"} overflow="hidden" mx="16px" bg={bg}>
        ...
        <ModalBody pb="4px">
          <Box p="14px" borderRadius="12px" bg="#fef2f2" border="1px solid #fca5a5" mb="16px">
            <Text fontSize="12.5px" color="#7f1d1d" fontWeight="500" lineHeight="1.7">
              ⚠️ Hành động này <strong>không thể hoàn tác</strong>. Toàn bộ dữ liệu của tài khoản
          ...
          >
            <Text fontSize="12px" color={isDark ? DARK.ink4 : "#64748b"} mb="2px">Tài khoản sẽ bị xóa:</Text>
            <Text fontSize="14px" fontWeight="700" color={ink}>{user.FullName}</Text>
            <Text fontSize="11px" color={isDark ? DARK.ink4 : "#94a3b8"} fontFamily="mono">ID: {user.UserId} · {user.Email}</Text>
          </Box>
          <Text fontSize="11.5px" color={isDark ? DARK.ink4 : "#64748b"} mb="8px" mt="14px">
            Nhập <strong style={{ color: "#dc2626", fontFamily: "monospace" }}>{user.UserId}</strong> để xác nhận:
          </Text>
          <Input {...inputSx(isDark)}
            ...
            borderColor={inputVal && inputVal !== user.UserId?.toString() ? "#fca5a5" : undefined}
          />
        </ModalBody>
        <ModalFooter gap="10px" pt="14px" pb="20px">
          ...
          <Button flex="1" h="42px" borderRadius="10px" fontWeight="700" fontSize="13px"
            bg={inputVal === user.UserId?.toString() ? "linear-gradient(135deg,#ef4444,#dc2626)" : (isDark ? DARK.ink6 : "#f1f5f9")}
            color={inputVal === user.UserId?.toString() ? "white" : (isDark ? DARK.ink4 : "#94a3b8")}
            boxShadow={inputVal === user.UserId?.toString() ? "0 4px 14px rgba(239,68,68,0.35)" : "none"}
